...
This page lists the capability set implemented and tested by the current release. The machine-readable response from `agent/ppm-agent.ps1 capabilities` is the runtime source of truth. Anything absent from both surfaces is outside the current support contract.
...
Initial setup changes host-wide UFW, swap/fstab, SMTP egress, SSH/sysctl/journald, package, unattended-upgrades, and vnstat state. Uninstall removes PPM-owned resources and named policy files; it leaves unknown prior global settings, installed packages, and swap in place.
...
| Link | Single-hop WireGuard over an isolated PPM interface/port/subnet |
...
The optional `mihomo-http` Provider accepts an HTTP or HTTPS source URL stored in local secret storage. PPM remains fully usable with zero Providers.
...
| `agent/ppm-agent.ps1` | Canonical sanitized JSON machine surface |
...
Read-only Route audit and sanitized desired-versus-observed drift cover PPM service/configuration, firewall/network, WireGuard, Hysteria2 listener/certificate, egress, ClientTarget render, and undetermined state.
