...
`version.txt` stores the Route Steward product version. Inventory schemas and dependency versions are tracked separately.
...
Release automation publishes the product version already present on `main`. Version changes happen in pull requests.
...
- leaves source and `version.txt` unchanged.

The repository `GITHUB_TOKEN` is the release credential.

If tag or release creation fails partway through, inspect the published state before rerunning. Never move an existing release tag to a different commit.
...
When no `v*` SemVer tag exists, start the first release with `workflow_dispatch`. Later version-changing merges publish automatically.
