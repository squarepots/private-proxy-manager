# Releasing Private Proxy Manager

`version.txt` is the only PPM product-version authority. Persisted inventory schema and dependency/protocol versions are separate compatibility domains; there is no independent object-model version axis.
...
- confirm `agent/ppm-agent.ps1 capabilities` reports the same product version as `version.txt`;
