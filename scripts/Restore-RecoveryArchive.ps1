...
$extract = Join-Path ([IO.Path]::GetTempPath()) ('ppm-recovery-extract-' + [Guid]::NewGuid().ToString('N'))
try {
    New-Item -ItemType Directory -Path $extract | Out-Null
    Protect-PPMPath -Path $extract -Directory
    ...
    Protect-PPMRecoveryTree -Root $extract
    $result = Restore-PPMExtractedRecovery -ExtractedDirectory $extract -PrivateDirectory $destination
