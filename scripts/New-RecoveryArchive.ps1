...
Protect-RSTPath -Path $recoveryDirectory -Directory
$stamp = [DateTime]::UtcNow.ToString('yyyyMMddTHHmmssZ')
$archive = Join-Path $recoveryDirectory "route-steward-recovery-$stamp.7z"
$stage = Join-Path ([IO.Path]::GetTempPath()) ('route-steward-recovery-' + [Guid]::NewGuid().ToString('N'))
$verified = $false
try {
    ...
    $startHere = @"
# Route Steward recovery

Give this encrypted recovery archive and the Route Steward repository to a supported capable AI agent and say:

> Recover my Route Steward state from this archive. Do not mutate remote infrastructure until the restored local state has been validated and audited.
...
"@
    ...
    Get-ChildItem -LiteralPath $stage -Recurse | ForEach-Object { Protect-RSTPath -Path $_.FullName -Directory:$_.PSIsContainer }
