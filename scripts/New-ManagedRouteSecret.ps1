...
if (-not (Test-PPMIPv4 $EntryIPv4)) { throw 'EntryIPv4 is invalid.' }
if ($EntryIPv6 -and -not (Test-PPMIPv6 $EntryIPv6)) { throw 'EntryIPv6 is invalid.' }
...
$stage = Join-Path ([IO.Path]::GetTempPath()) ('ppm-managed-secret-' + [Guid]::NewGuid().ToString('N'))
try {
...
"@
    ...
    Get-ChildItem -LiteralPath $stage -File | ForEach-Object { Protect-PPMPath -Path $_.FullName }
    ...
    Protect-PPMPath -Path $SecretDirectory -Directory
}
finally {
