...
NAME="PPM-Route"
PROXY_PORT="443"
OUTPUT="/var/lib/private-proxy-manager/client-payload.yaml"
ROTATE=0
CREDENTIAL_DIR=""
STATE_DIR="/var/lib/private-proxy-manager"
STATE_FILE="${STATE_DIR}/credentials.json"
HY_DIR="/etc/private-proxy-manager/hysteria"
HY_BIN="/usr/local/lib/private-proxy-manager/hysteria"
STAGE="$(mktemp -d /run/private-proxy-manager.XXXXXX)"
HY_PID=""

usage() {
  cat <<'EOF'
Usage: configure-proxies.sh --ipv4 ADDRESS [options]

Options:
  --ipv6 ADDRESS          Optional public IPv6 address.
  --name NAME             Safe node prefix (default: PPM-Route).
...
install -d -m 0750 -o ppm-hysteria -g ppm-hysteria "${HY_DIR}" "${HY_DIR}/tls" /var/lib/private-proxy-manager/hysteria
...
EOF
sed "s/^listen: :0$/listen: :${PROXY_PORT}/; s#cert: .*/server.crt#cert: ${HY_DIR}/tls/server.crt#; s#key: .*/server.key#key: ${HY_DIR}/tls/server.key#" "${STAGE}/hysteria-stage.yaml" >"${STAGE}/hysteria.yaml"
...
install -m 0600 -o ppm-hysteria -g ppm-hysteria "${STAGE}/server.key" "${HY_DIR}/tls/server.key"
install -m 0644 -o ppm-hysteria -g ppm-hysteria "${STAGE}/server.crt" "${HY_DIR}/tls/server.crt"
...
systemctl enable private-proxy-manager-hysteria.service >/dev/null
if ! systemctl is-active --quiet private-proxy-manager-hysteria.service; then
  systemctl start private-proxy-manager-hysteria.service
elif [[ "${RESTART_REQUIRED}" -eq 1 ]]; then
  systemctl restart private-proxy-manager-hysteria.service
fi
systemctl is-active --quiet private-proxy-manager-hysteria.service
