...
STAGE="$(mktemp -d /run/private-proxy-manager-relay-exit.XXXXXX)"
...
cat >"${STAGE}/99-private-proxy-manager-relay.conf" <<'EOF'
...
install -m 0644 -o root -g root "${STAGE}/99-private-proxy-manager-relay.conf" /etc/sysctl.d/99-private-proxy-manager-relay.conf
sysctl --system >/dev/null

ufw allow from "${ENTRY_PUBLIC_IP}" to any port "${LISTEN_PORT}" proto udp comment "PPM ${INTERFACE} WireGuard" >/dev/null
ufw allow in on "${INTERFACE}" comment "PPM ${INTERFACE} tunnel" >/dev/null
