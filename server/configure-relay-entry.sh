...
NAME=""
VIA_NAME=""
OUTPUT="/var/lib/route-steward/relay-client-payload.yaml"
UNIT_DIR=""
CREDENTIAL_DIR=""
HY_BIN="/usr/local/lib/route-steward/hysteria"
STAGE="$(mktemp -d /run/route-steward-relay-entry.XXXXXX)"
HY_PID=""
...
ROUTE_METRIC=42760
if [[ "${INTERFACE}" =~ ^wg-rst([0-9]{2})$ ]]; then ROUTE_METRIC=$((42760 + 10#${BASH_REMATCH[1]})); fi
...
STATE_DIR="/var/lib/route-steward/relays"
...
install -d -m 0750 -o route-steward-hysteria -g route-steward-hysteria "${HY_DIR}" /var/lib/route-steward/hysteria
...
install -m 0600 -o route-steward-hysteria -g route-steward-hysteria "${STAGE}/server.key" "${HY_DIR}/server.key"
install -m 0644 -o route-steward-hysteria -g route-steward-hysteria "${STAGE}/server.crt" "${HY_DIR}/server.crt"
install -m 0600 -o root -g root "${STAGE}/credentials.json" "${STATE_FILE}"
install -m 0644 "${UNIT_DIR}/route-steward-relay@.service" /etc/systemd/system/route-steward-relay@.service
DROP_IN="/etc/systemd/system/route-steward-relay@${NAME}.service.d"
...
if ! systemctl is-active --quiet "route-steward-relay@${NAME}.service"; then
  systemctl start "route-steward-relay@${NAME}.service"
elif [[ "${RESTART_REQUIRED}" -eq 1 ]]; then
  systemctl restart "route-steward-relay@${NAME}.service"
fi
systemctl is-active --quiet "route-steward-relay@${NAME}.service"
