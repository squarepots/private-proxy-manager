...
if [[ ! "${PROXY_PORT}" =~ ^[0-9]{1,5}$ ]] || ((PROXY_PORT < 1 || PROXY_PORT > 65535)); then echo "A valid proxy port is required." >&2; exit 2; fi
[[ "${EXPECTED_EXIT}" =~ ^[0-9]{1,3}(\.[0-9]{1,3}){3}$ ]] || { echo "Expected exit IPv4 is required." >&2; exit 2; }

service="private-proxy-manager-relay@${NAME}.service"
...
[[ "$(systemctl show -p User --value "${service}")" == ppm-hysteria ]] || fail_category remote-config-mismatch

state="/var/lib/private-proxy-manager/relays/${NAME}.json"
hy_dir="/etc/private-proxy-manager/hysteria/relays/${NAME}"
[[ -x "${PPM_BIN}" && -s "${state}" && -s "${hy_dir}/config.yaml" && -s "${hy_dir}/server.crt" ]] || fail_category remote-config-mismatch
...
if [[ -n "${EXPECTED_CONFIG_HASH}" ]]; then
  material="hysteria2-relay|port=${PROXY_PORT}|auth=${state_auth}|obfs=${state_obfs}|cert=${actual_fingerprint}|bind=${INTERFACE}"
