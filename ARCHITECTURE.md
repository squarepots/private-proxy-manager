...
Private Proxy Manager is a **model-agnostic, local-first infrastructure core operated by capable AI agents**. Natural-language conversation belongs to the host agent; durable infrastructure semantics belong to PPM.
...
Deterministic PPM core
  ...
  └─ migration/recovery primitives
          ↓
Private infrastructure and client artifacts
```

The core never branches on model/vendor identity. Runtime-specific files delegate to the same machine contract rather than creating separate PPM editions.
...
- **Route** — logical private exit offered to clients. A `direct` Route uses one Server; a `relay` Route references entry Server, exit Server, and Link. Initial ingress driver: `hysteria2`.
...
PPM does not publish compatibility machinery for development-only schemas that were never part of a public release. Recovery accepts the current public schema only. There is no independent persisted `model_version` axis.
...
Each new Link receives isolated PPM-native interface/UDP-port/subnet resources. Remote deployment and uninstall mutate PPM-owned resources and named policy files, while leaving unrelated firewall, WireGuard, proxy software, and other host state in place. The initial host preparation still has documented global effects; the supported host is dedicated and rebuildable.
...
- Mihomo ClientTargets use file delivery and may compose private Routes with explicitly selected generic Providers.
...
Current drift taxonomy distinguishes PPM service absence, remote configuration mismatch, firewall/network mismatch, WireGuard mismatch, Hysteria2 listener mismatch, certificate mismatch, egress mismatch, stale/missing ClientTarget renders, and undetermined state.
