...
param(
    ...
    [Parameter(Mandatory)][ValidateRange(1, 65535)][int]$ProxyPort,
...
. (Join-Path $repoRoot 'lib\PrivateProxyManager.Core.ps1')
...
function Get-ManagedRelayAuditExpectation {
    ...
    $credential = Read-PPMJson -Path $credentialPath -Label 'Managed relay credentials'
    $auth = [string](Get-PPMOptional (Get-PPMOptional $credential 'hysteria') 'auth')
    $obfs = [string](Get-PPMOptional (Get-PPMOptional $credential 'hysteria') 'obfs')
...
function Write-SanitizedAuditOutput {
    ...
    $hasCategory = @($Output | Where-Object { $_ -match '^PPM_AUDIT_CATEGORY=' }).Count -gt 0
    if (-not $hasCategory) { $Output += 'PPM_AUDIT_CATEGORY=undetermined' }
    foreach ($line in $Output) {
        if ($line -match '^(?:PPM_AUDIT_CATEGORY|RELAY_EGRESS_IPV4|HYSTERIA_VERSION|WIREGUARD_VERSION|RELAY_(?:ENTRY|EXIT)_AUDIT_OK|RELAY_EGRESS_OK)=') { Write-Output $line }
    }
    Write-Output "PPM_AUDIT_EXIT_CODE=$ExitCode"
}

$ssh = Get-Command ssh -ErrorAction SilentlyContinue
$scp = Get-Command scp -ErrorAction SilentlyContinue
if (-not $ssh -or -not $scp) { throw 'OpenSSH client commands ssh and scp are required.' }
if (-not (Test-PPMIPv4 $EntryHost)) { throw 'EntryHost must be an IPv4 address.' }
if (-not (Test-PPMIPv4 $ExitHost)) { throw 'ExitHost must be an IPv4 address.' }
if ($EntryHost -eq $ExitHost) { throw 'Relay entry and exit must be different servers.' }
if ($EntryIPv6 -and -not (Test-PPMIPv6 $EntryIPv6)) { throw 'EntryIPv6 must be an IPv6 address.' }
...
if (-not (Test-PPMIPv4 $entryPeerIp) -or -not (Test-PPMIPv4 $exitPeerIp)) { throw 'Tunnel endpoint addresses are invalid.' }
...
$auditExpectation = Get-ManagedRelayAuditExpectation -Directory $CredentialBundleDirectory -Port $ProxyPort -BindInterface $Interface
...
if ($LinkSecretPath) {
    ...
    if (-not $AuditOnly) {
        $localSecretStage = Join-Path ([IO.Path]::GetTempPath()) ('ppm-link-secret-' + [Guid]::NewGuid().ToString('N'))
        New-Item -ItemType Directory -Path $localSecretStage | Out-Null
        Protect-PPMPath -Path $localSecretStage -Directory
        foreach ($side in 'entry','exit') {
...
$exitTarget = $ExitHost
$entryRoot = '/tmp/private-proxy-manager-relay-entry-' + [Guid]::NewGuid().ToString('N')
$exitRoot = '/tmp/private-proxy-manager-relay-exit-' + [Guid]::NewGuid().ToString('N')
...
try {
    ...
    if (-not $AuditOnly) {
        ...
        Invoke-Native $ssh.Source ($entrySsh + @($entryTarget, (ConvertTo-BashCommand @('sudo','bash',"$entryServer/install-proxy-binaries.sh","$entryServer/config"))))
        ...
        $entryConfigure = @('sudo','bash',"$entryServer/configure-relay-entry.sh",'--interface',$Interface,'--local-cidr',$EntryTunnelAddress,'--peer-ip',$exitPeerIp,'--peer-public-key',$exitPublicKey,'--exit-endpoint',$ExitHost,'--tunnel-port',[string]$TunnelPort,'--proxy-port',[string]$ProxyPort,'--entry-ipv4',$EntryHost,'--exit-ipv4',$ExitHost,'--name',$Name,'--via-name',$ViaName,'--output','/var/lib/private-proxy-manager/relay-client-payload.yaml','--unit-dir',"$entryServer/config")
        ...
        Protect-PPMPath -Path $privateDir -Directory
        $remoteExport = "$entryRoot/relay-client-payload.yaml"
        $copyCommand = ConvertTo-BashCommand @('sudo','install','-m','0600','-o',$EntryUser,'/var/lib/private-proxy-manager/relay-client-payload.yaml',$remoteExport)
        ...
        Protect-PPMPath -Path $localPayload
    }
    ...
    $entryAudit = @('sudo','bash',"$entryServer/audit-relay.sh",'--role','entry','--interface',$Interface,'--name',$Name,'--proxy-port',[string]$ProxyPort,'--peer-ip',$exitPeerIp,'--expected-exit',$ExitHost)
