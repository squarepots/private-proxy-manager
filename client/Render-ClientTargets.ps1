...
function Read-RouteNodes {
    ...
    foreach ($path in $PayloadPaths) {
        ...
        if ($raw -notmatch '(?m)^schema:\s*1\s*$' -or $raw -notmatch '(?ms)^proxies:\s*\r?\n(?<nodes>.+)\z') { throw 'Every route payload must be a PPM schema 1 client payload.' }
...
function Get-ProfileRoutePayloads {
    ...
    return @($routes | ForEach-Object {
        $driver = [string](Get-PPMOptional (Get-PPMOptional $_ 'ingress') 'driver' 'hysteria2')
        if ($driver -ne 'hysteria2') { throw "Route '$($_.id)' uses unsupported ingress driver '$driver' for this renderer." }
        Resolve-PPMSecret -Reference ([string]$_.payload_secret_ref) -PrivateDirectory $PrivateDirectory
    })
}

function Get-ProfileProviders {
    param([Parameter(Mandatory)]$Inventory, [Parameter(Mandatory)]$Profile, [Parameter(Mandatory)][string]$PrivateDirectory)
    $include = @(Get-PPMOptional $Profile 'include_providers' @())
    if (-not $include.Count) { return @() }
    $providers = @($Inventory.providers | Where-Object { (Get-PPMOptional $_ 'enabled' $true) -ne $false })
    if ($include -notcontains '*') { $providers = @($providers | Where-Object { $include -contains $_.id }) }
    return @($providers | ForEach-Object {
        $sourceType = [string](Get-PPMOptional $_ 'source_type')
        if ($sourceType -ne 'mihomo-http') { throw "Provider '$($_.id)' source type '$sourceType' is unsupported by the Mihomo renderer." }
        $reference = [string](Get-PPMOptional $_ 'source_secret_ref')
        $urlPath = Resolve-PPMSecret -Reference $reference -PrivateDirectory $PrivateDirectory
        ...
        [pscustomobject]@{ Id = [string]$_.id; Name = [string](Get-PPMOptional $_ 'display_name' $_.id); Url = $url; Interval = [int](Get-PPMOptional $_ 'interval_seconds' 86400) }
    })
}
...
function Render-Mihomo {
    ...
    $policy = [string](Get-PPMOptional $Profile 'policy' 'privacy')
...
"@
    ...
    $text = @"
# private-proxy-manager: agent-native
...
"@
    ...
    if (-not $SkipValidation) {
        ...
        if ($core) {
            $validationHome = Join-Path ([IO.Path]::GetTempPath()) ('ppm-mihomo-' + [Guid]::NewGuid().ToString('N'))
            try {
...
function Get-SubscriptionUrl {
    ...
    $doc = Read-PPMJson -Path $path -Label 'The private subscription secret'
...
Protect-PPMPath -Path $OutputDirectory -Directory
