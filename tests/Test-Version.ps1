...
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) ('rst-version-test-' + [Guid]::NewGuid().ToString('N'))
...
$capabilitiesRaw = & (Join-Path $repo 'agent\route-steward-agent.ps1') capabilities 2>&1
...
Write-Host "Product version behavior passed for Route Steward $version."
