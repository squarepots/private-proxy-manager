...
$stage = Join-Path ([IO.Path]::GetTempPath()) ('ppm-observed-test-' + [Guid]::NewGuid().ToString('N'))
try {
    ...
    foreach ($case in @(
        ...
        $null = Set-PPMObservedRoute -Inventory $inventory -PrivateDirectory $stage -RouteId route-a -Status $case.Status -Category $case.Category -ActualEgressIPv4 '192.0.2.44' -HysteriaVersion 'v-test'
    ...
    $null = Set-PPMObservedRoute -Inventory $inventory -PrivateDirectory $stage -RouteId route-a -Status healthy -Category in-sync -ActualEgressIPv4 '192.0.2.44' -HysteriaVersion 'v-test'
    ...
    $observed = Read-PPMObservedState -PrivateDirectory $stage
