...
$stage = Join-Path ([IO.Path]::GetTempPath()) ('ppm-provider-test-' + [Guid]::NewGuid().ToString('N'))
try {
    ...
    $secretPath = Resolve-PPMSecret -Reference ([string]$inventory.providers[0].source_secret_ref) -PrivateDirectory $stage
