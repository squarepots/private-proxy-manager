...
$stage = Join-Path ([IO.Path]::GetTempPath()) ('ppm-core-test-' + [Guid]::NewGuid().ToString('N'))
try {
    $state = Initialize-PPMPrivateState -PrivateDirectory $stage
    ...
    $badSchema.schema = 2
    $failed = $false
    try { $null = Assert-PPMInventory -Inventory $badSchema -PrivateDirectory $stage -SkipSecretCheck }
    ...
    $null = Add-PPMServer -Inventory $inventory -InventoryPath $inventoryPath -Context $serverContext
    $inventory = Read-PPMInventory -Path $inventoryPath
    ...
    foreach ($badUser in @('-oProxyCommand=bad','--help','bad@user','bad user',"bad`nuser",('a' * 33))) {
        Assert-True (-not (Test-PPMUnixUser $badUser)) "Unsafe SSH user '$badUser' passed the core validator."
    }
    foreach ($goodUser in @('ubuntu','root','user_1')) { Assert-True (Test-PPMUnixUser $goodUser) "Supported SSH user '$goodUser' was rejected." }

    $allocation = New-PPMLinkAllocation -Inventory $inventory
    Assert-True ($allocation.interface -eq 'wg-ppm01' -and $allocation.listen_port -eq 51820 -and $allocation.subnet -eq '10.77.1.0/30') 'PPM-native Link allocation is incorrect.'
    ...
    $null = Add-PPMRoute -Inventory $inventory -InventoryPath $inventoryPath -Context $routeContext
    $inventory = Read-PPMInventory -Path $inventoryPath
    ...
    $profile = Add-PPMProfile -Inventory $inventory -InventoryPath $inventoryPath -Context $profileContext
    Assert-True ([string]$profile.policy -eq '') 'A Profile silently selected a geographic policy.'
    $inventory = Read-PPMInventory -Path $inventoryPath
    $target = Add-PPMClientTarget -Inventory $inventory -InventoryPath $inventoryPath -Context ([pscustomobject][ordered]@{ target_id = 'desktop'; profile_id = 'primary'; renderer = 'mihomo' })
    ...
    $failed = $false
    try { $null = Assert-PPMInventory -Inventory $badListener -PrivateDirectory $stage -SkipSecretCheck }
    ...
    $failed = $false
    try { $null = Assert-PPMInventory -Inventory $badDriver -PrivateDirectory $stage -SkipSecretCheck }
    ...
    $null = Add-PPMProvider -Inventory $inventory -InventoryPath $inventoryPath -Context $providerContext
    $inventory = Read-PPMInventory -Path $inventoryPath
    ...
    if ($env:OS -ne 'Windows_NT') {
        Assert-True (Test-PPMPrivateAcl -Path $stage) 'POSIX private directory permissions are too broad.'
        Assert-True (Test-PPMPrivateAcl -Path $inventoryPath) 'POSIX inventory permissions are too broad.'
        Assert-True (Test-PPMPrivateAcl -Path (Join-Path $stage 'secrets\index.json')) 'POSIX secret index permissions are too broad.'
    }
