...
$root = Join-Path ([IO.Path]::GetTempPath()) ('rst-recovery-core-test-' + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    $inventory = New-RSTCleanInventory -PrivateDirectory (Join-Path $source 'private')
    ...
    $metadata = [ordered]@{ schema = 1; product = 'route-steward'; inventory_schema = 1; recovery_model = 'agent-native-local-state' }
    ...
    $result = Restore-RSTExtractedRecovery -ExtractedDirectory $source -PrivateDirectory $target
    ...
    $restored = Read-RSTInventory -Path (Join-Path $target 'inventory.json')
    ...
    Assert-True (Test-RSTPrivateAcl -Path $restoredKey) 'Recovered SSH key permissions are not owner-only.'
    ...
    $observed = Read-RSTJson -Path (Join-Path $target 'observed.json') -Label 'Restored observed state'
    ...
    $failed = $false
    try { $null = Restore-RSTExtractedRecovery -ExtractedDirectory $tampered -PrivateDirectory $tamperedTarget }
