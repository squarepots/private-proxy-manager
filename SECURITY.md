...
Private Proxy Manager operates private network infrastructure from a trusted local controller. Its security contract covers vulnerability reporting, authority, secret handling, preflight, remote ownership, credential changes, migration, and recovery.
...
Use [GitHub private vulnerability reporting](https://github.com/squarepots/private-proxy-manager/security/advisories/new) for a sensitive vulnerability.
...
PPM trusts the local user account that owns private state and the local tool-capable runtime chosen by that user.
...
PPM applies current-user-only ACLs on Windows and owner-only modes on Unix-like systems. Private state remains plaintext unless the operating system, disk, or backup layer encrypts it.
...
- `remote-write` for deterministic PPM deployment;
...
- `private-proxy-manager-*` systemd units;
- the `ppm-hysteria` runtime identity;
- `wg-ppm*` interfaces and files;
- explicitly PPM-named policy files.

Deployment and uninstall stay inside this ownership boundary. Unrelated proxy software, WireGuard configuration, services, accounts, packages, firewall rules, and host files remain untouched. Uninstall removes PPM-owned artifacts and named policy files; it does not reconstruct unknown earlier global host settings.
...
Cloudflare can process the subscription response and request metadata within its role. A leaked subscription token exposes that target's configuration body, not SSH or PPM management authority.
