...
function Test-RSTRecoveryManifest {
...
function Protect-RSTRecoveryTree {
    param([Parameter(Mandatory)][string]$Root)
    Protect-RSTPath -Path $Root -Directory
    Get-ChildItem -LiteralPath $Root -Recurse -Force | ForEach-Object { Protect-RSTPath -Path $_.FullName -Directory:$_.PSIsContainer }
}

function Restore-RSTExtractedRecovery {
    param(
    ...
    $verifiedFiles = Test-RSTRecoveryManifest -ExtractedDirectory $sourceRoot
    ...
    if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf) -or -not (Test-Path -LiteralPath $sourceInventory -PathType Leaf) -or -not (Test-Path -LiteralPath (Join-Path $sourceSecrets 'index.json') -PathType Leaf)) {
        throw 'Recovery archive does not contain complete RST canonical state.'
    }
    $metadata = Read-RSTJson -Path $metadataPath -Label 'Recovery metadata'
    $metadataSchema = [int](Get-RSTOptional $metadata 'inventory_schema' 0)
    if ([string](Get-RSTOptional $metadata 'product') -ne 'route-steward' -or $metadataSchema -ne 1) {
        throw 'Recovery archive metadata is not compatible with this RST recovery workflow.'
    }

    $inventory = Read-RSTJson -Path $sourceInventory -Label 'Recovered inventory'
    $sourceSchema = [int](Get-RSTOptional $inventory 'schema' 0)
    ...
    $stage = Join-Path $parent ('.rst-restore-' + [Guid]::NewGuid().ToString('N'))
    $moved = $false
    $completed = $false
    try {
        ...
        Write-RSTJsonAtomic -Value $inventory -Path (Join-Path $stage 'inventory.json')
        Write-RSTJsonAtomic -Value ([ordered]@{ schema = 1; generated_at = $null; servers = @(); links = @(); routes = @() }) -Path (Join-Path $stage 'observed.json')
        Protect-RSTRecoveryTree -Root $stage

        $validationInventory = Read-RSTInventory -Path (Join-Path $stage 'inventory.json')
        ...
        $moved = $true
        Protect-RSTRecoveryTree -Root $target
        $finalInventory = Read-RSTInventory -Path (Join-Path $target 'inventory.json')
        foreach ($server in @($finalInventory.servers)) {
            ...
            if (-not (Test-RSTPrivateAcl -Path ([string]$server.ssh.key_path))) { throw 'Recovered SSH key permissions are not private.' }
        }
        $completed = $true
        return [ordered]@{
            restored = $true
