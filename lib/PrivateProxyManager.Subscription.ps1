...
# implementation in PrivateProxyManager.SubscriptionTargets.ps1. Lifecycle and
# state ownership intentionally do not live in this helper layer.

function New-PPMSubscriptionToken {
    $bytes = [byte[]]::new(32)
    [Security.Cryptography.RandomNumberGenerator]::Fill($bytes)
    return [Convert]::ToBase64String($bytes).TrimEnd('=').Replace('+','-').Replace('/','_')
}

function Get-PPMSha256Hex {
...
function Assert-PPMWorkerIdentity {
...
function Test-PPMSubscriptionEndpoint {
    param([Parameter(Mandatory)][string]$Url, [Parameter(Mandatory)][string]$ExpectedBody)
    try { $response = Invoke-WebRequest -Uri $Url -Method GET -TimeoutSec 30 -Headers @{ 'User-Agent' = 'PrivateProxyManager/1' } }
