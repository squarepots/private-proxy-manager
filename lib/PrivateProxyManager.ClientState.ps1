...
function Get-PPMClientCanonicalFingerprint {
    param(
    ...
    try {
        function Add-PPMFingerprintPart {
        ...
        Add-PPMFingerprintPart -Name 'inventory.json' -Bytes ([IO.File]::ReadAllBytes($inventoryFull))
        foreach ($file in @(Get-ChildItem -LiteralPath $secretsRoot -Recurse -Force -File | Sort-Object FullName)) {
            ...
            Add-PPMFingerprintPart -Name ('secrets/' + $relative.Replace('\','/')) -Bytes ([IO.File]::ReadAllBytes($file.FullName))
        }
...
function Get-PPMClientRenderManifestPath {
...
function Read-PPMClientRenderManifest {
    param([Parameter(Mandatory)][string]$PrivateDirectory, [switch]$AllowMissing)
    $path = Get-PPMClientRenderManifestPath -PrivateDirectory $PrivateDirectory
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        if ($AllowMissing) { return [pscustomobject][ordered]@{ schema = 1; targets = @() } }
        throw 'Client render manifest is missing.'
    }
    $manifest = Read-PPMJson -Path $path -Label 'Client render manifest'
    if ([int](Get-PPMOptional $manifest 'schema' 0) -ne 1) { throw 'Client render manifest schema must be 1.' }
    return $manifest
}

function Update-PPMClientRenderManifest {
    param(
    ...
    $manifest = Read-PPMClientRenderManifest -PrivateDirectory $PrivateDirectory -AllowMissing
    $fingerprint = Get-PPMClientCanonicalFingerprint -InventoryPath $InventoryPath -PrivateDirectory $PrivateDirectory
    $map = @{}
    foreach ($entry in @(Get-PPMOptional $manifest 'targets' @())) { $map[[string]$entry.id] = $entry }
    foreach ($output in $Outputs) {
        $targetId = [string](Get-PPMOptional $output 'client_target')
        $path = [string](Get-PPMOptional $output 'path')
    ...
    $validIds = @(Get-PPMClientTargets -Inventory $Inventory | ForEach-Object { [string]$_.id })
    $next = [ordered]@{
        schema = 1
        targets = @($map.Keys | Where-Object { $validIds -contains $_ } | Sort-Object | ForEach-Object { $map[$_] })
    }
    Write-PPMJsonAtomic -Value $next -Path (Get-PPMClientRenderManifestPath -PrivateDirectory $PrivateDirectory)
    return $next
}

function Get-PPMClientRenderDrift {
    param(
        [Parameter(Mandatory)]$Inventory,
        [Parameter(Mandatory)][string]$InventoryPath,
        [Parameter(Mandatory)][string]$PrivateDirectory
    ...
    $map = @{}
    foreach ($entry in @(Get-PPMOptional $manifest 'targets' @())) { $map[[string]$entry.id] = $entry }
    $delivery = [string](Get-PPMOptional $Inventory.delivery 'directory' (Join-Path $PrivateDirectory 'delivery'))
    $items = [Collections.Generic.List[object]]::new()
    foreach ($target in @(Get-PPMClientTargets -Inventory $Inventory | Sort-Object id)) {
        $id = [string]$target.id
        $entry = $map[$id]
        $category = 'in-sync'
        if (-not $entry) { $category = 'client-render-stale' }
        elseif ([string](Get-PPMOptional $entry 'input_fingerprint') -ne $fingerprint) { $category = 'client-render-stale' }
        else {
            $fileName = [string](Get-PPMOptional $entry 'file_name')
            if (-not $fileName -or [IO.Path]::GetFileName($fileName) -ne $fileName) { $category = 'client-render-stale' }
                ...
                if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { $category = 'client-render-stale' }
                    ...
                    if ($hash -ne [string](Get-PPMOptional $entry 'output_sha256')) { $category = 'client-render-stale' }
                }
            }
        }
        $items.Add([ordered]@{
