...
. (Join-Path $PSScriptRoot 'PrivateProxyManager.Version.ps1')

function Get-PPMCapabilityCatalog {
    $catalog = @(
...
function Get-PPMCapabilityById {
    param([Parameter(Mandatory)][string]$Id)
    $matches = @(Get-PPMCapabilityCatalog | Where-Object { $_.id -eq $Id })
    if ($matches.Count -ne 1) { throw "Unsupported PPM operation '$Id'." }
    return $matches[0]
}

function Get-PPMDriverCapabilities {
    return [ordered]@{
        schema_version = 1
...
function Initialize-PPMPrivateState {
...
function Get-PPMSanitizedContext {
    ...
    return [ordered]@{
        ...
        counts = [ordered]@{
            ...
            links = @(Get-PPMOptional $Inventory 'links' @()).Count
            routes = @(Get-PPMOptional $Inventory 'routes' @()).Count
            enabled_routes = $enabledRoutes.Count
            providers = @(Get-PPMOptional $Inventory 'providers' @()).Count
            enabled_providers = $enabledProviders.Count
            profiles = $profiles.Count
            client_targets = $targets.Count
        }
        profiles = @($profiles | ForEach-Object { [ordered]@{ id = [string]$_.id; policy = [string](Get-PPMOptional $_ 'policy') } })
        client_targets = @($targets | ForEach-Object { [ordered]@{ id = [string]$_.id; profile = [string](Get-PPMOptional $_ 'profile'); renderer = [string](Get-PPMOptional $_ 'renderer'); delivery = [string](Get-PPMOptional $_ 'delivery') } })
        supported_operations = @(Get-PPMCapabilityCatalog | ForEach-Object { [ordered]@{ id = $_.id; state = $_.state; authorization_class = $_.authorization_class } })
    }
}

function Get-PPMTargetRoute {
...
function New-PPMPreflight {
    ...
    $capability = Get-PPMCapabilityById -Id $Operation
    ...
    switch ($Operation) {
        ...
        'audit' {
            ...
            elseif (-not (Get-PPMTargetRoute -Inventory $Inventory -Target $Target)) { $conflicts.Add('target-route-missing') }
        ...
        'add-server' {
            ...
            if ($Context -and (Get-PPMOptional $Context 'ssh_user') -and -not (Test-PPMUnixUser ([string]$Context.ssh_user))) { $conflicts.Add('ssh-user-invalid') }
            if ($Context -and (Get-PPMOptional $Context 'host_ownership') -and [string]$Context.host_ownership -ne 'dedicated') { $conflicts.Add('host-ownership-unsupported') }
            $effects.Add('update-local-desired-state')
        }
        'add-link' {
            foreach ($field in 'link_id','entry_server','exit_server') { if (-not $Context -or -not (Get-PPMOptional $Context $field)) { $missing.Add($field.Replace('_','-')) } }
            if ($Context) {
                if ((Get-PPMOptional $Context 'entry_server') -and -not (Get-PPMTargetServer -Inventory $Inventory -Target ([string]$Context.entry_server))) { $conflicts.Add('entry-server-missing') }
                if ((Get-PPMOptional $Context 'exit_server') -and -not (Get-PPMTargetServer -Inventory $Inventory -Target ([string]$Context.exit_server))) { $conflicts.Add('exit-server-missing') }
                if ((Get-PPMOptional $Context 'entry_server') -and (Get-PPMOptional $Context 'entry_server') -eq (Get-PPMOptional $Context 'exit_server')) { $conflicts.Add('link-endpoints-must-differ') }
            }
            $effects.Add('allocate-local-link-and-keys')
        }
        'add-route' {
            foreach ($field in 'route_id','kind','entry_server') { if (-not $Context -or -not (Get-PPMOptional $Context $field)) { $missing.Add($field.Replace('_','-')) } }
            if ($Context) {
                $kind = [string](Get-PPMOptional $Context 'kind')
                if ($kind -notin @('direct','relay')) { $conflicts.Add('unsupported-route-kind') }
                if ((Get-PPMOptional $Context 'entry_server') -and -not (Get-PPMTargetServer -Inventory $Inventory -Target ([string]$Context.entry_server))) { $conflicts.Add('entry-server-missing') }
                if ($kind -eq 'relay') {
                    if (-not (Get-PPMOptional $Context 'exit_server')) { $missing.Add('exit-server') }
                    if (-not (Get-PPMOptional $Context 'link_id')) { $missing.Add('link-id') }
                }
            }
            $effects.Add('generate-local-route-credentials-and-update-desired-state')
        }
        'add-provider' {
            if (-not $Context -or -not (Get-PPMOptional $Context 'provider_id')) { $missing.Add('provider-id') }
            if (-not $Context -or -not (Get-PPMOptional $Context 'url')) { $missing.Add('provider-url') }
            if ($Context -and (Get-PPMOptional $Context 'provider_id')) {
                $id = ConvertTo-PPMId ([string]$Context.provider_id)
                if (Get-PPMProviderById -Inventory $Inventory -Id $id -AllowMissing) { $conflicts.Add('provider-id-already-exists') }
            }
        ...
        'update-provider' {
            ...
            elseif (-not (Get-PPMProviderById -Inventory $Inventory -Id $Target -AllowMissing)) { $conflicts.Add('target-provider-missing') }
        ...
        'remove-provider' {
            if (-not $Target) { $missing.Add('target-provider') }
                ...
                $provider = Get-PPMProviderById -Inventory $Inventory -Id $Target -AllowMissing
                if (-not $provider) { $conflicts.Add('target-provider-missing') }
                if (@($Inventory.profiles | Where-Object { @(Get-PPMOptional $_ 'include_providers' @()) -contains $Target }).Count) { $conflicts.Add('provider-still-referenced-by-profile') }
            }
            $effects.Add('remove-local-provider-state-and-url-secret')
        }
        'add-profile' {
            if (-not $Context -or -not (Get-PPMOptional $Context 'profile_id')) { $missing.Add('profile-id') }
            elseif (Get-PPMProfileById -Inventory $Inventory -Id (ConvertTo-PPMId ([string]$Context.profile_id)) -AllowMissing) { $conflicts.Add('profile-id-already-exists') }
            $effects.Add('update-local-profile-selection')
        }
        'update-profile' {
            if (-not $Target) { $missing.Add('target-profile') }
            elseif (-not (Get-PPMProfileById -Inventory $Inventory -Id $Target -AllowMissing)) { $conflicts.Add('target-profile-missing') }
        ...
        'remove-profile' {
            if (-not $Target) { $missing.Add('target-profile') }
            elseif (-not (Get-PPMProfileById -Inventory $Inventory -Id $Target -AllowMissing)) { $conflicts.Add('target-profile-missing') }
            elseif (@(Get-PPMClientTargets -Inventory $Inventory | Where-Object { $_.profile -eq $Target }).Count) { $conflicts.Add('profile-still-referenced-by-client-target') }
            $effects.Add('remove-local-profile')
        }
        'add-client-target' {
            foreach ($field in 'target_id','profile_id','renderer') { if (-not $Context -or -not (Get-PPMOptional $Context $field)) { $missing.Add($field.Replace('_','-')) } }
            if ($Context -and (Get-PPMOptional $Context 'target_id')) {
                $id = ConvertTo-PPMId ([string]$Context.target_id)
                if (Get-PPMClientTargetById -Inventory $Inventory -Id $id -AllowMissing) { $conflicts.Add('client-target-id-already-exists') }
            }
            if ($Context -and (Get-PPMOptional $Context 'profile_id') -and -not (Get-PPMProfileById -Inventory $Inventory -Id ([string]$Context.profile_id) -AllowMissing)) { $conflicts.Add('client-target-profile-missing') }
            if ($Context -and (Get-PPMOptional $Context 'renderer') -and [string]$Context.renderer -notin @('mihomo','shadowrocket')) { $conflicts.Add('client-target-renderer-unsupported') }
            $effects.Add('add-local-client-target')
        }
        'update-client-target' {
            if (-not $Target) { $missing.Add('target-client-target') }
            elseif (-not (Get-PPMClientTargetById -Inventory $Inventory -Id $Target -AllowMissing)) { $conflicts.Add('target-client-target-missing') }
        ...
        'remove-client-target' {
            if (-not $Target) { $missing.Add('target-client-target') }
                ...
                $clientTarget = Get-PPMClientTargetById -Inventory $Inventory -Id $Target -AllowMissing
                if (-not $clientTarget) { $conflicts.Add('target-client-target-missing') }
                elseif (Get-PPMOptional $clientTarget 'subscription_secret_ref') { $conflicts.Add('client-target-has-subscription-state') }
            }
            $effects.Add('remove-local-client-target')
        }
        'deploy-route' {
            $route = Get-PPMTargetRoute -Inventory $Inventory -Target $Target
            if (-not $route) { $missing.Add('target-route') }
            else {
                foreach ($ref in @($route.payload_secret_ref, (Get-PPMOptional $route 'credential_secret_ref')) | Where-Object { $_ }) { try { $null = Resolve-PPMSecret -Reference $ref -PrivateDirectory $PrivateDirectory } catch { $conflicts.Add('required-secret-missing') } }
                foreach ($serverId in @($route.entry_server, $route.exit_server) | Sort-Object -Unique) {
                    $server = Get-PPMTargetServer -Inventory $Inventory -Target ([string]$serverId)
                    if (-not $server) { $conflicts.Add('server-missing'); continue }
                    if ([string](Get-PPMOptional (Get-PPMOptional $server 'compute') 'host_ownership') -ne 'dedicated') { $conflicts.Add('dedicated-host-not-confirmed') }
                    if (-not (Test-PPMUnixUser ([string](Get-PPMOptional (Get-PPMOptional $server 'ssh') 'user')))) { $conflicts.Add('ssh-user-invalid') }
        ...
        'render-client' {
            if ($Target -and -not (Get-PPMClientTargetById -Inventory $Inventory -Id $Target -AllowMissing)) { $conflicts.Add('target-client-target-missing') }
            if (@($Inventory.routes | Where-Object { (Get-PPMOptional $_ 'enabled' $true) -ne $false }).Count -eq 0) { $missing.Add('enabled-route') }
        ...
        'publish-subscription' {
            if (-not $Target) { $missing.Add('target-client-target') }
                ...
                if (-not $clientTarget -or [string]$clientTarget.renderer -ne 'shadowrocket') { $conflicts.Add('target-client-target-not-shadowrocket') }
                    ...
                    $reference = [string](Get-PPMOptional $clientTarget 'subscription_secret_ref')
                    if (-not $reference) {
                        if (-not $Context -or -not (Get-PPMOptional $Context 'worker_name')) { $missing.Add('worker-name') }
                        if (-not $Context -or -not (Get-PPMOptional $Context 'host')) { $missing.Add('host') }
                    }
                    else { try { $null = Resolve-PPMSecret -Reference $reference -PrivateDirectory $PrivateDirectory } catch { $conflicts.Add('subscription-state-missing') } }
                }
            }
            if (@($Inventory.routes | Where-Object { (Get-PPMOptional $_ 'enabled' $true) -ne $false }).Count -eq 0) { $missing.Add('enabled-route') }
        ...
        'rotate-subscription-token' {
            if (-not $Target) { $missing.Add('target-client-target') }
                ...
                elseif (-not (Get-PPMOptional $clientTarget 'subscription_secret_ref')) { $conflicts.Add('client-target-has-no-subscription-state') }
            }
        ...
        'migrate-route' {
            $route = Get-PPMTargetRoute -Inventory $Inventory -Target $Target
            if (-not $route) { $missing.Add('target-route') }
            if (-not $Context -or -not (Get-PPMOptional $Context 'replacement_server_id')) { $missing.Add('replacement-server') }
            elseif (-not (Get-PPMTargetServer -Inventory $Inventory -Target ([string]$Context.replacement_server_id))) { $conflicts.Add('replacement-server-not-in-inventory') }
...
function Add-PPMServer {
    ...
    $id = ConvertTo-PPMId ([string]$Context.server_id)
    if (Get-PPMTargetServer -Inventory $Inventory -Target $id) { throw "Server '$id' already exists." }
    $publicV4 = [string]$Context.public_ipv4
    if (-not (Test-PPMIPv4 $publicV4)) { throw 'public_ipv4 is invalid.' }
    $publicV6 = [string](Get-PPMOptional $Context 'public_ipv6')
    if ($publicV6 -and -not (Test-PPMIPv6 $publicV6)) { throw 'public_ipv6 is invalid.' }
    $privateV4 = [string](Get-PPMOptional $Context 'private_ipv4')
    if ($privateV4 -and -not (Test-PPMIPv4 $privateV4)) { throw 'private_ipv4 is invalid.' }
    $sshUser = [string](Get-PPMOptional $Context 'ssh_user')
    if (-not (Test-PPMUnixUser $sshUser)) { throw 'ssh_user must be a lowercase Unix account name with at most 32 characters.' }
    $hostOwnership = [string](Get-PPMOptional $Context 'host_ownership')
    ...
    $server = [pscustomobject][ordered]@{
        id = $id
        provider = [string](Get-PPMOptional $Context 'provider' 'byo')
        account_label = [string](Get-PPMOptional $Context 'account_label' 'personal')
        instance_name = [string](Get-PPMOptional $Context 'instance_name' $id)
        region = [string](Get-PPMOptional $Context 'region' 'unknown')
        zone = [string](Get-PPMOptional $Context 'zone' '')
        os = [string](Get-PPMOptional $Context 'os' 'ubuntu-24.04')
        architecture = [string](Get-PPMOptional $Context 'architecture' 'x86_64')
        roles = @(Get-PPMOptional $Context 'roles' @('entry','exit'))
        compute = [pscustomobject][ordered]@{ driver = 'byo-ssh'; host_ownership = $hostOwnership }
        network = [pscustomobject][ordered]@{ public_ipv4 = $publicV4; ipv4_type = 'static'; private_ipv4 = $privateV4; public_ipv6 = $publicV6; expected_egress_ipv4 = [string](Get-PPMOptional $Context 'expected_egress_ipv4' $publicV4); expected_egress_ipv6 = Get-PPMOptional $Context 'expected_egress_ipv6' }
        ssh = [pscustomobject][ordered]@{ user = $sshUser; key_path = $keyPath; allowed_sources = @(Get-PPMOptional $Context 'ssh_allowed_sources' @('trusted')) }
    ...
    Save-PPMInventory -Inventory $Inventory -InventoryPath $InventoryPath -SkipSecretCheck
    return $server
}

function Add-PPMLink {
    param([Parameter(Mandatory)]$Inventory, [Parameter(Mandatory)][string]$InventoryPath, [Parameter(Mandatory)]$Context)
    $repoRoot = Split-Path -Parent $PSScriptRoot
    $privateDirectory = Split-Path -Parent ([IO.Path]::GetFullPath($InventoryPath))
    $id = ConvertTo-PPMId ([string]$Context.link_id)
    $entry = [string]$Context.entry_server; $exit = [string]$Context.exit_server
    $null = Get-PPMServerById -Inventory $Inventory -Id $entry; $null = Get-PPMServerById -Inventory $Inventory -Id $exit
    ...
    $allocation = New-PPMLinkAllocation -Inventory $Inventory
    ...
    try {
        ...
        Write-PPMJsonAtomic -Value $secretIndex -Path $indexPath; $indexWritten = $true
        Save-PPMInventory -Inventory $candidate -InventoryPath $InventoryPath
    } catch {
        if ($indexWritten) { $secretIndex.refs.PSObject.Properties.Remove($secretReference); Write-PPMJsonAtomic -Value $secretIndex -Path $indexPath }
...
function Add-PPMRoute {
    param([Parameter(Mandatory)]$Inventory, [Parameter(Mandatory)][string]$InventoryPath, [Parameter(Mandatory)]$Context)
    $repoRoot = Split-Path -Parent $PSScriptRoot
    $privateDirectory = Split-Path -Parent ([IO.Path]::GetFullPath($InventoryPath))
    $id = ConvertTo-PPMId ([string]$Context.route_id)
    ...
    $entry = [string]$Context.entry_server; $entryServer = Get-PPMServerById -Inventory $Inventory -Id $entry
    $exit = if ($kind -eq 'direct') { $entry } else { [string]$Context.exit_server }; $null = Get-PPMServerById -Inventory $Inventory -Id $exit
    $link = if ($kind -eq 'relay') { [string]$Context.link_id } else { $null }
    if ($kind -eq 'relay') { $linkObject = Get-PPMLinkById -Inventory $Inventory -Id $link; if ($linkObject.entry_server -ne $entry -or $linkObject.exit_server -ne $exit) { throw 'The Link does not match Route endpoints.' } }
    ...
    $port = [int](Get-PPMOptional $Context 'listen_port' $defaultPort); if ($port -lt 1 -or $port -gt 65535) { throw 'listen_port is invalid.' }
    $displayName = [string](Get-PPMOptional $Context 'display_name' $id); if ($displayName -notmatch '^[A-Za-z0-9._-]+$') { throw 'display_name must use ASCII letters, digits, dot, underscore, or dash.' }
    ...
    $null = Assert-PPMInventory -Inventory $candidate -PrivateDirectory $privateDirectory -SkipSecretCheck
    $secretIndex = Read-PPMSecretIndex -PrivateDirectory $privateDirectory; $indexPath = Join-Path $privateDirectory 'secrets\index.json'; $indexWritten = $false
    ...
    try {
        & (Join-Path $repoRoot 'scripts\New-ManagedRouteSecret.ps1') -RouteId $id -DisplayName $displayName -EntryIPv4 ([string]$entryServer.network.public_ipv4) -EntryIPv6 ([string](Get-PPMOptional $entryServer.network 'public_ipv6')) -Port $port -SecretDirectory $managedDirectory
        ...
        if ($indexWritten) { $secretIndex.refs.PSObject.Properties.Remove($payloadRef); $secretIndex.refs.PSObject.Properties.Remove($credentialRef); Write-PPMJsonAtomic -Value $secretIndex -Path $indexPath }
        if (Test-Path -LiteralPath $managedDirectory) { Remove-Item -LiteralPath $managedDirectory -Recurse -Force }
        throw
    }
    return $route
