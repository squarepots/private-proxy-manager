...
function Write-PPMJsonAtomic {
    ...
    try {
        ...
        Protect-PPMPath -Path $temporary
        Move-Item -LiteralPath $temporary -Destination $fullPath -Force
        Protect-PPMPath -Path $fullPath
    }
    finally {
...
function Test-PPMUnixUser {
    param([AllowNull()][string]$Value)
    if ([string]::IsNullOrWhiteSpace($Value)) { return $false }
    return $Value -cmatch '^[a-z_][a-z0-9_-]{0,31}$'
}

function Read-PPMJson {
...
function Get-PPMDefaultPolicies {
    return @(
        [ordered]@{
            id = 'balanced-cn'
...
function Assert-PPMInventory {
    param([Parameter(Mandatory)]$Inventory, [Parameter(Mandatory)][string]$PrivateDirectory, [switch]$SkipSecretCheck)
    $failures = [Collections.Generic.List[string]]::new()
    if ([int](Get-PPMOptional $Inventory 'schema' 0) -ne 1) { $failures.Add('Inventory schema must be 1.') }

    $servers = @(Get-PPMOptional $Inventory 'servers' @())
    $links = @(Get-PPMOptional $Inventory 'links' @())
    $routes = @(Get-PPMOptional $Inventory 'routes' @())
    $providers = @(Get-PPMOptional $Inventory 'providers' @())
    $policies = @(Get-PPMOptional $Inventory 'policies' @())
    $profiles = @(Get-PPMOptional $Inventory 'profiles' @())
    $clientTargets = @(Get-PPMOptional $Inventory 'client_targets' @())
    ...
    foreach ($collection in @(
        ...
        $ids = @($collection.Values | ForEach-Object { [string](Get-PPMOptional $_ 'id') })
    ...
    foreach ($server in $servers) {
        ...
        $compute = Get-PPMOptional $server 'compute'
        if ([string](Get-PPMOptional $compute 'driver') -ne 'byo-ssh') {
            $failures.Add("Server '$id' must declare compute.driver=byo-ssh.")
        }
        if ([string](Get-PPMOptional $compute 'host_ownership') -ne 'dedicated') {
            $failures.Add("Server '$id' must declare compute.host_ownership=dedicated.")
        }
        $network = Get-PPMOptional $server 'network'
        $publicV4 = [string](Get-PPMOptional $network 'public_ipv4')
        $publicV6 = [string](Get-PPMOptional $network 'public_ipv6')
        $privateV4 = [string](Get-PPMOptional $network 'private_ipv4')
        $egressV4 = [string](Get-PPMOptional $network 'expected_egress_ipv4')
        if ($publicV4 -and -not (Test-PPMIPv4 $publicV4)) { $failures.Add("Server '$id' has an invalid public IPv4.") }
        if ($privateV4 -and -not (Test-PPMIPv4 $privateV4)) { $failures.Add("Server '$id' has an invalid private IPv4.") }
        if ($publicV6 -and -not (Test-PPMIPv6 $publicV6)) { $failures.Add("Server '$id' has an invalid public IPv6.") }
        if ($egressV4 -and -not (Test-PPMIPv4 $egressV4)) { $failures.Add("Server '$id' has an invalid expected egress IPv4.") }
        $ssh = Get-PPMOptional $server 'ssh'
        $sshUser = [string](Get-PPMOptional $ssh 'user')
        if (-not $sshUser -or -not (Get-PPMOptional $ssh 'key_path')) {
            $failures.Add("Server '$id' must have an SSH user and key path.")
        }
        elseif (-not (Test-PPMUnixUser $sshUser)) { $failures.Add("Server '$id' has an invalid SSH user.") }
    }

    foreach ($server in $servers) {
        $seenFirewallRules = @{}
        foreach ($rule in @(Get-PPMOptional (Get-PPMOptional $server 'firewall') 'rules' @())) {
            $family = [string](Get-PPMOptional $rule 'family')
            $protocol = [string](Get-PPMOptional $rule 'protocol')
            $port = [int](Get-PPMOptional $rule 'port' 0)
            $sourceServer = [string](Get-PPMOptional $rule 'source_server')
            $source = [string](Get-PPMOptional $rule 'source')
    ...
    foreach ($link in $links) {
        $id = [string]$link.id
        $linkById[$id] = $link
        if ([string](Get-PPMOptional $link 'type') -ne 'wireguard') { $failures.Add("Link '$id' must use WireGuard.") }
        if ([string](Get-PPMOptional $link 'driver') -ne 'wireguard') { $failures.Add("Link '$id' must declare driver=wireguard.") }
        foreach ($endpoint in 'entry_server', 'exit_server') {
            $serverId = [string](Get-PPMOptional $link $endpoint)
            if (-not $serverById.ContainsKey($serverId)) { $failures.Add("Link '$id' references unknown $endpoint '$serverId'.") }
        }
        $interface = [string](Get-PPMOptional $link 'interface')
        $port = [int](Get-PPMOptional $link 'listen_port' 0)
        $subnet = [string](Get-PPMOptional $link 'subnet')
    ...
    foreach ($route in $routes) {
        $id = [string]$route.id
        $kind = [string](Get-PPMOptional $route 'kind')
        if ($kind -notin @('direct', 'relay')) { $failures.Add("Route '$id' has unsupported kind '$kind'.") }
        if ([string](Get-PPMOptional (Get-PPMOptional $route 'ingress') 'driver') -ne 'hysteria2') {
            $failures.Add("Route '$id' must declare ingress.driver=hysteria2.")
        }
        $entry = [string](Get-PPMOptional $route 'entry_server')
        $exit = [string](Get-PPMOptional $route 'exit_server')
        ...
        $port = [int](Get-PPMOptional $route 'listen_port' 0)
        ...
        if ($kind -eq 'relay') {
            $linkId = [string](Get-PPMOptional $route 'link')
        ...
        if (-not (Get-PPMOptional $route 'payload_secret_ref')) { $failures.Add("Route '$id' is missing its payload secret reference.") }
    }

    $providerIds = @($providers | ForEach-Object { [string]$_.id })
    foreach ($provider in $providers) {
        $id = [string]$provider.id
        if ([string](Get-PPMOptional $provider 'source_type') -ne 'mihomo-http') { $failures.Add("Provider '$id' must declare source_type=mihomo-http.") }
        if (-not (Get-PPMOptional $provider 'source_secret_ref')) { $failures.Add("Provider '$id' is missing source_secret_ref.") }
        if ([int](Get-PPMOptional $provider 'interval_seconds' 0) -lt 3600) { $failures.Add("Provider '$id' refresh interval must be at least one hour.") }
        if ((Get-PPMOptional $provider 'health_check' $false) -ne $false) { $failures.Add("Provider '$id' must not enable automatic health checking.") }
    }
    ...
    foreach ($target in $clientTargets) {
        $id = [string]$target.id
        $profileId = [string](Get-PPMOptional $target 'profile')
        $renderer = [string](Get-PPMOptional $target 'renderer')
        $delivery = [string](Get-PPMOptional $target 'delivery')
        $subscriptionRef = [string](Get-PPMOptional $target 'subscription_secret_ref')
    ...
    if (-not $SkipSecretCheck) {
        try {
            ...
            $references = @()
            $references += $providers | Where-Object { (Get-PPMOptional $_ 'enabled' $true) -ne $false } | ForEach-Object { Get-PPMOptional $_ 'source_secret_ref' }
            $references += $routes | Where-Object { (Get-PPMOptional $_ 'enabled' $true) -ne $false } | ForEach-Object { Get-PPMOptional $_ 'payload_secret_ref' }
            $references += $routes | Where-Object { (Get-PPMOptional $_ 'enabled' $true) -ne $false } | ForEach-Object { Get-PPMOptional $_ 'credential_secret_ref' }
            $references += $links | Where-Object { (Get-PPMOptional $_ 'enabled' $true) -ne $false } | ForEach-Object { Get-PPMOptional $_ 'secret_ref' }
            $references += $clientTargets | ForEach-Object { Get-PPMOptional $_ 'subscription_secret_ref' }
            foreach ($reference in @($references | Where-Object { $_ } | Sort-Object -Unique)) {
                try { $null = Resolve-PPMSecret -Reference $reference -PrivateDirectory $PrivateDirectory -Index $secretIndex }
...
function Read-PPMInventory {
    param([Parameter(Mandatory)][string]$Path, [switch]$SkipSecretCheck)
    $inventory = Read-PPMJson -Path $Path -Label 'The private inventory'
...
function New-PPMLinkAllocation {
    param([Parameter(Mandatory)]$Inventory)
    $usedInterfaces = @(@(Get-PPMOptional $Inventory 'links' @()) | ForEach-Object { [string]$_.interface })
    $usedPorts = @(@(Get-PPMOptional $Inventory 'links' @()) | ForEach-Object { [int]$_.listen_port })
    $usedSubnets = @(@(Get-PPMOptional $Inventory 'links' @()) | ForEach-Object { [string]$_.subnet })
    for ($slot = 1; $slot -le 99; $slot++) {
        $interface = 'wg-ppm{0:d2}' -f $slot
        $port = 51819 + $slot
