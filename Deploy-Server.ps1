...
param(
    ...
    [ValidateRange(1, 65535)][int]$ProxyPort = 443,
...
function Get-ManagedAuditExpectation {
    ...
    $credential = Read-PPMJson -Path $credentialPath -Label 'Managed route credentials'
...
if (-not (Test-PPMIPv4 $ServerHost)) { throw 'ServerHost must be an IPv4 address.' }
if ($IPv6 -and -not (Test-PPMIPv6 $IPv6)) { throw 'IPv6 must be an IPv6 address.' }
...
$auditExpectation = Get-ManagedAuditExpectation -Directory $CredentialBundleDirectory -Port $ProxyPort
$serverDir = Join-Path $repoRoot 'server'
$remoteRoot = '/tmp/private-proxy-manager-' + ([Guid]::NewGuid().ToString('N'))
$target = $ServerHost
...
try {
    ...
    if (-not $AuditOnly) {
        ...
        Invoke-Native $ssh.Source ($sshCommon + @($target, (ConvertTo-BashCommand @('sudo','bash',"$remoteServer/install-proxy-binaries.sh","$remoteServer/config"))))
        ...
        $configure = @('sudo','bash',"$remoteServer/configure-proxies.sh",'--ipv4',$ServerHost,'--name',$Name,'--port',[string]$ProxyPort,'--output','/var/lib/private-proxy-manager/client-payload.yaml')
        ...
        $copyCommand = ConvertTo-BashCommand @('sudo','install','-m','0600','-o',$User,'/var/lib/private-proxy-manager/client-payload.yaml',$remoteExport)
    ...
    $audit = @('sudo','bash',"$remoteServer/audit.sh",'--proxy-port',[string]$ProxyPort)
    if ($auditExpectation) {
    ...
    $category = @($auditOutput | Where-Object { $_ -match '^PPM_AUDIT_CATEGORY=' } | Select-Object -Last 1)
    if (-not $category.Count) { $auditOutput += 'PPM_AUDIT_CATEGORY=undetermined' }
    foreach ($line in $auditOutput) {
        if ($line -match '^(?:PPM_AUDIT_CATEGORY|IPV4|HYSTERIA_VERSION|WIREGUARD_VERSION|AUDIT_OK)=') { Write-Output $line }
    }
    Write-Output "PPM_AUDIT_EXIT_CODE=$auditExit"
