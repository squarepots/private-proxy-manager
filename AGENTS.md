...
Private Proxy Manager (PPM) is agent-native infrastructure software. A user should be able to give a capable agent this repository URL, describe the route they want, and let the agent discover the supported workflow.

## First actions

1. Read `.agents/skills/private-proxy-manager/SKILL.md`.
2. Inspect `agent/ppm-agent.ps1 capabilities` before assuming support.
3. Run `scripts/Validate-Local.ps1 -Quick` before using real infrastructure.
4. Bootstrap only when private state is absent.
5. Read sanitized context and drift before changing an existing setup.
6. Run scoped preflight before every mutation and execute only when `ready=true`.
7. Validate meaningful effects and report a sanitized result.
...
`agent/ppm-agent.ps1` is the canonical sanitized machine surface. `mcp/` is a thin local-stdio adapter. Lower-level PowerShell, render, deploy, audit, and Worker code are implementation surfaces for agents and contributors.
...
Prefer repository-owned deterministic operations. Raw SSH is limited to read-only diagnosis when no bounded PPM diagnostic exists; it must not become an alternate configuration system.
...
Remote deployment and uninstall may mutate only PPM-owned resources and named policy files. Unrelated proxy software, WireGuard state, services, accounts, and host files remain outside PPM ownership. Uninstall does not reconstruct unknown prior global host settings.

## Capability and documentation ownership

`docs/COMPATIBILITY.md` and capability discovery define implemented support. External software documentation may provide evidence but does not add a PPM driver, renderer, or operation.
