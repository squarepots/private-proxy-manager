...
func Capabilities() []Capability {
	...
	return []Capability{
		...
		capability("add-client-target", "agent", true, "local-write", "Add a renderer-backed ClientTarget that references a reusable Profile.", []ContextField{field("target_id", "stable-id", true), field("profile_id", "profile-id", true), field("renderer", "mihomo|shadowrocket|hysteria2", true), {Name: "route_id", Type: "route-id", Required: false, When: "renderer is hysteria2"}, {Name: "listen", Type: "loopback-listener", Required: false, When: "renderer is hysteria2"}, {Name: "ingress_family", Type: "auto|ipv4|ipv6", Required: false, When: "renderer is hysteria2"}}, "update-local-client-target"),
...
func DriverCapabilities() map[string]any {
	return map[string]any{
		...
		"renderers":             []any{map[string]any{"id": "mihomo", "state": "supported", "clients": []string{"Clash Verge-compatible Mihomo clients"}}, map[string]any{"id": "shadowrocket", "state": "supported", "delivery": []string{"node-import", "private-subscription"}}, map[string]any{"id": "hysteria2", "state": "supported", "delivery": []string{"private-json"}, "local_modes": []string{"http", "socks5"}, "runtime": "verified-official-client"}},
